...
import { applyDemoTokenBalanceOverrides } from '../../util/demoMode/balanceOverrides';
...
export const getTokenBalancesControllerTokenBalances = createDeepEqualSelector(
  [
    ...
    (state) =>
      state.engine?.backgroundState?.AccountsController?.internalAccounts
        ?.accounts ?? {},
    (state) => state.demoMode?.tokenBalanceOverrides ?? [],
  ],
  (
    isAssetsUnifyStateEnabled: boolean,
    ...
    customAssets: AssetsControllerState['customAssets'],
    internalAccountsById: AccountsControllerState['internalAccounts']['accounts'],
    demoBalanceOverrides,
  ): TokenBalancesControllerState['tokenBalances'] => {
    const evmAccountAddresses = Object.values(internalAccountsById)
      .filter((account) => isEvmAccountType(account.type))
      .map((account) => account.address as Hex);

    const withDemoOverrides = (
      balances: TokenBalancesControllerState['tokenBalances'],
    ) => {
      if (!isDemoModeEnabled() || demoBalanceOverrides.length === 0) {
        return balances;
      }
      return applyDemoTokenBalanceOverrides(
        balances,
        demoBalanceOverrides,
        evmAccountAddresses,
      );
    ...
    if (!isAssetsUnifyStateEnabled) {
      return withDemoOverrides(tokenBalances);
    }
    ...
    return withDemoOverrides(result);
  },
);
